...
const strategy_params = require("../strategy_params.json");
const params = strategy_params[0];

//Account To Impersonate
const ownersAccount = "0xFd525F21C17f2469B730a118E0568B4b459d61B9"; //DALLAS Account

const want_token_holder = "0x14B2e8329b8e06BCD524eb114E23fAbD21910109";
...
let strategy;
let pool_id;
let pending_gamma_in_old_strat;
let pending_gamma_in_new_strat;
...
describe("STRATEGY CONTRACT DEPLOYMENT", function () {
  it("Should Deploy Strategy", async function () {
    ...
    const Strategy = await ethers.getContractFactory(
      "contracts/uranus_strategy_old.sol:GammaStrategy_Uranus"
    );
...
describe("ADD DEPLOYED STRATEGY TO THE FARM", function () {
  it("should revert with `Ownable: caller is not the owner`", async function () {
    const farm = await ethers.getContractAt(
      "contracts/farm.sol:PlanetFinance",
      farm_Address
    );
...
describe("TRANSFER WANT BALANCE INTO THE USER2 account", function () {
  it("should transfer and check the want balance of `want_token_holder` address to user2`", async function () {
    const want_inst = await ethers.getContractAt(
      "contracts/aqua.sol:AQUA",
      want
    );

    const want_token_holder_want_bal = await want_inst.balanceOf(
      want_token_holder
    );

    if (want_token_holder_want_bal > 0) {
      const signer = await ethers.getSigner(want_token_holder);
      ...
      await want_inst
        .connect(signer)
        .transfer(user2.address, want_token_holder_want_bal);
      ...
      const user2_want_token_bal = await want_inst.balanceOf(user2.address);

      expect(want_token_holder_want_bal).to.equal(user2_want_token_bal);
    }
  });
});
...
describe(`DEPOSIT ${
  ...
  it("should approve the farm to spend user2 want tokens", async function () {
    ...
    if (depositAmount > allowance) {
      await want_inst
        .connect(user2)
        .approve(farm_Address, BigInt(Math.pow(2, 256)) - BigInt(1));
    }
  });

  it(`should deposit user2 ${
    depositAmount / BigInt(1e18)
  } WANT `, async function () {
    const farm = await ethers.getContractAt(
      "contracts/farm.sol:PlanetFinance",
  ...
  it(`should deposit user2 ${
    depositAmount / BigInt(1e18)
  } WANT AGAIN AFTER 1 hour`, async function () {
...
describe(`TEST WITHDRAW ${
  ...
  it("should print user2 pending gamma after withdraw after 1 block", async function () {
    ...
    pending_gamma_in_old_strat = (
      await strategy.userPendingGammaProfit(user2.address)
    ).pendingProfitInGamma;
...
/********************************************COPY ABOVE CODE FOR NEW WANT STRATEGY**************************************/
...
describe("STRATEGY CONTRACT DEPLOYMENT", function () {
  it("Should Deploy Strategy", async function () {
    ...
    owner = await ethers.getSigner(ownersAccount);
    const Strategy = await ethers.getContractFactory(
      "contracts/uranus_strategy_new.sol:GammaStrategy_Uranus"
    );
    strategy = await Strategy.deploy(
      params[0],
      params[1],
...
describe(`PRINT PENDING GAMMA RESULTS FROM BOTH OLD AND NEW STRATEGIES`, function () {
  it("PRINT", async function () {
    console.log(
      "\n PENDING GAMMA FOR OLD STRATEGY : ",
      pending_gamma_in_old_strat
    );
    console.log(
      "\n PENDING GAMMA FOR NEW STRATEGY : ",
      pending_gamma_in_new_strat
    );
  });
});
